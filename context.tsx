...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateRegisteredUser = (updatedUser: User) => {
    const users = getRegisteredUsers();
  ...
  const register = async (name: string, email: string, pass: string) => {
      ...
      const newUser: User = {
          ...
          name: name,
          email: email,
          password: pass, // Storing simply for this demo environment without backend
          avatar: ''
      };
  ...
  const logout = () => {
      saveData({ user: null });
      // We don't clear the whole DB, just the session
  };
  ...
  const updateUser = async (userData: Partial<User>) => {
      ...
      if (dbUser) {
          updateRegisteredUser({ ...dbUser, ...userData });
      }
  };
  ...
  const changePassword = async (currentPass: string, newPass: string) => {
      ...
      updateRegisteredUser({ ...dbUser, password: newPass });
      setLoading(false);
  };
  ...
  const addBudget = async (budget: Orcamento) => {
      saveData({ budgets: [budget, ...budgets] });
  };
  ...
  return (
    <AppContext.Provider value={{ 
      user, loading, materials, budgets, checklist, measurements, additions, integrations,
